# -*- usage : scrapy runspider detik_v2.py -s JOB_DIR=. -o 2018.json
import scrapy
import datetime
import lxml
...
class JPSpider(scrapy.Spider):
    name = "jakartapost"
    allowed_domains = ["jakartapost.com"]
    ...
    def start_requests(self):
        ...
        for date in date_generated:
            ...
            #https://www.thejakartapost.com/search?q=2017%2F01%2F01
            datestr = date.strftime("%Y/%d/%m")
            req = Request("https://www.thejakartapost.com/search?q="+datestr,  callback = self.check_maxpage)
            req.meta['date'] = datestr
            req.meta['url'] = "https://www.thejakartapost.com/search?q="+datestr
            print(req)
    ...
    def check_maxpage(self, response):
        ...
        max = int(response.css("div.gsc-cursor-page").extract()
